window.MethodologyOrchestratorService = Object.seal({
  ...
  stages: [
    ...
    { id: "EXECUTIVE_REPORT", name: "Executive Report", required: true, owner: "Executive Report Generator" }
  ],
  ...
  loadSourceStates() {
    return {
      ...
      executiveReport: window.ExecutiveReportService ? window.ExecutiveReportService.loadState() : null
    };
  },
  ...
  resolveReadiness(stageId, states) {
    const resolvers = {
      ...
      EXECUTIVE_REPORT: () => this.executiveReportReadiness(states.executiveReport)
    };
  ...
  executiveReportReadiness(state) {
    const packageData = state && state.executiveReportPackage ? state.executiveReportPackage : null;
    const questions = state && state.questions ? state.questions : [];
    const blockingQuestions = questions.filter((item) => item.status === "OPEN" && item.blocksConsolidation);
    const ready = Boolean(packageData && packageData.sections && packageData.sections.length && !blockingQuestions.length);

    return {
      ready,
      progress: packageData ? (ready ? 100 : 70) : 0,
      healthScore: packageData ? (ready ? 86 : 62) : 0,
      missingInformation: packageData ? blockingQuestions.map((item) => item.question) : ["Executive Report Package no disponible."],
      updatedAt: packageData ? packageData.createdAt : ""
    };
  },
